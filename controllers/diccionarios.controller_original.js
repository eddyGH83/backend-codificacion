...
const devuelveCatalogoPorCodigo= async (req, res) => {
	...
	const query = {
		text: `SELECT * FROM ${esquema}.cod_catalogo WHERE catalogo ILIKE $1 
		AND estado ILIKE 'ACTIVO' and codigo = $2 order by id_catalogo desc `,
		values: [
			params.catalogo,
			params.codigo
		],
...
const validarMatriz = async (req, res) => {
	...
	const query = {
		text: `select * from ${esquema}.cod_matriz 
			...
			and descripcion_ocupacion ilike '${params.descripcion_ocupacion}' and descripcion_acteco ilike '${params.decripcion_acteco}' and
			estado ilike 'ACTIVO'`,
		/*values: [
...
const devuelveCorrector = async (req, res) => {
	let params = req.body;
	const query = {
		text: `SELECT * FROM ${esquema}.cod_err_corr order by id desc`,
	};
	await con
		.query(query)
		.then((result) =>
...
const updateEstadoDiccCorr = async (req, res) => {
	...
	let params = req.body;
	const query = {
		text: `UPDATE ${esquema}.cod_err_corr SET estado=$1, usumod=$2, femod=now() WHERE id=${id}`,
		values: [
			params.estado,
			params.user
		],
